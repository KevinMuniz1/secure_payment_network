...
export default function Login() {
  ...
  const handleSubmit = async (e: React.SyntheticEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (data.requiresTotp || data.requiresEmailOtp) {
        sessionStorage.setItem('preAuthToken', data.preAuthToken)
        sessionStorage.setItem('twoFactorMethod', data.requiresTotp ? 'totp' : 'email')
        navigate('/verify-2fa')
        return
      }
  ...
  return (
    <div className="min-h-screen flex" style={{ backgroundColor: '#080808' }}>
      ...
      >
        ...
        <div>
          <h1 className="text-white font-light text-4xl leading-snug mb-6" style={{ letterSpacing: '-0.5px' }}>
            Banking built for<br />those who expect more.
          </h1>
          <div style={{ borderTop: '1px solid #1e1e1e', paddingTop: '24px' }} className="space-y-3">
            {[
              ...
              <div key={f} className="flex items-center gap-3">
                <div className="w-1 h-1" style={{ backgroundColor: '#c9a96e', borderRadius: 0 }} />
                <p className="text-xs tracking-wide" style={{ color: '#666' }}>{f}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      >
        <div className="w-full max-w-sm">
          ...
          <p className="mt-8 text-xs" style={{ color: '#555' }}>
            New to SecurePay?{' '}
            <Link to="/register" className="font-medium transition-colors" style={{ color: '#c9a96e' }}>
              Open an account →
            </Link>
          </p>
        </div>
